 ...
 * {@code IPMatcher} is initialized with a particular IP range specification. Calls to {@link IPMatcher#match(String) match}
 * method will then quickly determine whether a given IP falls within that range. Supported range specifications: - Full
 * IPv4 address, e.g. {@code 12.34.56.78} - Full IPv6 address, e.g. {@code 2001:18e8:3:171:218:8bff:fe2a:56a4} - Partial
 * IPv4 address, e.g. {@code 12.34} (which matches any IP starting {@code 12.34}) - IPv4 network/netmask, e.g. {@code 18.25.0.0/255.255.0.0}notation,
 * e.g. {@code 18.25.0.0/16}, {@code 2001:18e8:3:171::/64}
 *
 * @version $Revision$
...
public class IPMatcher {
   ...
   private String ipSpecification = null;

   /**
    * Construct an IPMatcher that will test for the given IP specification
    *
    * @param ipSpec IP specification (full or partial address, network/netmask, network/cidr)
    * @throws IPMatcherException if there is an error parsing the specification (i.e. it is somehow malformed)
    */
   public IPMatcher(String ipSpec) throws IPMatcherException {
      setup(ipSpec);
   }

   public String getIpSpecification() {
      return ipSpecification;
   }

   public void setIpSpecification(String ipSpec) throws IPMatcherException {
      setup(ipSpec);
   }

   private void setup(String ipSpec) throws IPMatcherException {
      // Boil all specs down to network + mask
      ipSpecification = null;
         ...
         switch (parts.length) {
            case 2:
             ...
             case 1:
               ...
               partCount = ipToBytes(ipPart, network, mustHave4);
    ...
    * @param mustHave4 if true, will require that the given IP string specify all four bytes
    * @return the number of actual IP bytes found in the given IP address String
    * @throws IPMatcherException if there is a problem parsing the IP string -- e.g. number outside of range 0-255, too
    * many numbers, less than 4 numbers if {@code mustHave4} is true
    */
    ...
    * @throws IPMatcherException if the IP passed in cannot be parsed correctly (i.e. is malformed)
    */
