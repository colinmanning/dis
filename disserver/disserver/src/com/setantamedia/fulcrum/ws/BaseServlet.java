...
public abstract class BaseServlet extends HttpServlet {
    ...
    public final static String PARAMETER_EXACT = "exact";
    public final static String PARAMETER_DETAILED = "detailed";
    ...
    public final static String CONTENT_TYPE_FORM = "application/x-www-formd-urlencoded";
    ...
    public final static String ADDRESS_LOOOPBACK = "127.0.0.1";
    ...
    protected IPMatcher allowedIP = null;
    ...
    protected boolean requestAllowed(String ip) {
        ...
        try {
            if (publicAccess || isLocalhost(ip)) {
                result = true;
            } else if (allowedIP != null && allowedIP.match(ip)) {
                result = true;
            } else {
                // check if access for service is allowed
                Connection con = dam.connections.get(connectionName);
                if (con == null) {
                    logger.error("Cannot find connection with name: '" + connectionName + "'");
                } else {
        ...
        if (!result) {
            logger.info("Unauthorized attempt to access DIS service from IP address: " + ip);
        }
        return result;
    }
