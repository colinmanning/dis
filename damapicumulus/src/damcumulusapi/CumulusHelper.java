...
public class CumulusHelper {
    ...
    private HashMap<Connection, DatabaseField[]> categoryFields = new HashMap<>();
    private HashMap<Connection, HashMap<String, DatabaseField>> categoryFieldsByName = new HashMap<>();
    ...
    private GUID[] categoryFieldGuids = null;
            ...
            for (int f = 0; f < fields.length; f++) {
                try {
                    ...
                    if (guidS != null && !"".equals(guidS)) {
                        categoryFieldGuids[f] = new GUID(guidS);
                    }
                } catch (Exception e) {
                    // do nothing
                }
    ...
    public String getCategoryFieldGuid(Connection connection, String fieldName) {
        String result = null;
        try {
            result = categoryFieldsByName.get(connection).get(fieldName).getGuid().toString();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
            ...
            if (recordId != -1) {
                ...
                if (hostIds == null) {
                    ...
                    try {
                        ...
                        for (Map.Entry<String, String> entry : fieldValues.entrySet()) {
                            ...
                            try {
                                ...
                                logger.error("Problem setting value for field: '" + fieldName + "' continuing");
                    ...
                    try {
                        ...
                        // TODO - this not really implemented or thought - need to set
                        // foreign key fields etc.
                        // parent is a table field, get the record first, then the
                        // field
                        // We must traverse the parent ids, assuming a root
                        // AssetRecord
                        ...
                        if (rootFieldDef != null) {
                            ...
                            for (Map.Entry<String, String> entry : fieldValues.entrySet()) {
                                // logger.debug("COLIN>>>>> updating sub table field: "
                                // + entry.getKey());
    ...
    public Person userSingleSignOn(Connection connection, String username) {
        ...
        try {
            ...
            for (UserFieldDefinition fieldDef : fieldDefs) {
                ...
                // String fieldName =
                // CumulusUtilities.normaliseFieldName(fieldDef.getName(Cumulus.getLanguageID()));
            ...
            // logger.debug("attempt to validate connection: '" +
            // connection.toString() + " failed");
    ...
    public void checkAllCategoryFieldsReady(Connection connection) {
        if (categoryFields.get(connection) == null) {
            getCategoryFields(connection);
        }
    }

    public DatabaseField[] getCategoryFields(Connection connection) {
        DatabaseField[] result = null;
        result = categoryFields.get(connection);
        if (result != null) {
            return result;
        }
        result = new DatabaseField[0];
        CumulusCollectionManager collectionManager = null;
        ItemCollection col = null;
        try {
            collectionManager = getOrInitCollectionManager(connection);
            if (collectionManager.getOnDemand()) {
                // we need to open a connection, and close it later
                col = collectionManager.borrowOnDemand();
            }
            Layout layout = collectionManager.getCategoryLayout();
            ...
            if (col != null) {
                collectionManager.returnObject(col, false);
            }
        }
    ...
    public DatabaseField[] getFields(Connection connection, String viewName) {
        ...
        try {
            ...
            for (GUID guid : guids) {
                ...
                field.setValueInterpretation(fieldDefinition.getValueInterpretation());
                field.setGuid((guid == null) ? null : guid.toString());
                // logger.debug("Database field named '" + field.getName() +
                // "' has type: " + field.getDataType() + " and interpretation: " +
                // field.getValueInterpretation()+ " in database: " +
    ...
    public Category[] matchCategories(Connection connection, String query, boolean detailed, boolean recursive) {
        ...
        try {
            ...
            Layout layout = collectionManager.getCategoryLayout();
            getCategoryFieldGuids(getCategoryFields(connection));
            if (collectionManager != null) {
                try {
                    //System.out.println("Match Categories Query is: '"+query+"'");
                    AllCategoriesItemCollection allCategoriesItemCollection = collectionManager.getAllCategoriesItemCollection();
                    if (allCategoriesItemCollection != null) {
                        synchronized (allCategoriesItemCollection) {
                            try {
                                List<Integer> ids = allCategoriesItemCollection.getCategoryItemIDs(query, EnumSet.of(FindFlag.FIND_MISSING_FIELDS_REPLACE));
                                result = new Category[ids.size()];
                                int i = 0;
                                for (Integer id : ids) {
                    ...
                    logger.info("problem processing category match  query: '" + query + "'");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    protected void setFieldValue(Item item, GUID guid, FieldValue fieldValue, Layout layout) throws Exception {
        switch (fieldValue.getDataType()) {
            ...
            case FieldTypeConstants.TypeDate:
                // logger.debug("got date value to set: " +
                // fieldValue.dateTimeValue.getValue());
                // logger.debug("   --- java date value is: " + new
                // Date(fieldValue.dateTimeValue.getValue()));
                ...
                switch (fieldValue.getValueInterpretation()) {
                    ...
                    case FieldTypeConstants.VALUE_STRING_ENUM_MULTIPLE_VALUES:
                        ...
                        if (ev.supportsMultipleValues() && fieldValue.getStringListValue().length > 0) {
                            ...
                            if (ev.hasValue()) {
                                ids = ev.getIDs();
                                // we assume all values to be overwritten, so first clear the
                                // set
                                ids.clear();
                            }
                ...
                switch (layout.getFieldDefinition(guid).getValueInterpretation()) {
                    case FieldTypeConstants.VALUE_DEFAULT:
                        ...
                        if (item.hasValue(guid)) {
                            ...
                            try {
                                // may have no value, so catch and ignore (like a
                                // "hasValue(guid)" check
                    ...
                    case FieldTypeConstants.VALUE_STRING_ENUM_MULTIPLE_VALUES:
                        if (item.hasValue(guid)) {
                            ...
                            if (ids.size() > 0) {
                                ...
                                // return a single null value, so client code does not break
                                // if no value set
                            ...
                            // return a single null value, so client code does not break if
                            // no value set
                            StringListValue[] slv = new StringListValue[1];
                            slv[0] = new StringListValue();
                            result.setValueInterpretation(FieldTypeConstants.VALUE_STRING_ENUM_MULTIPLE_VALUES);
            ...
            if (recordItem != null) {
                ...
                if (recordItem.hasValue(GUID.UID_REC_ASSET_REFERENCE)) {
                    ...
                    try {
                        ...
                        // JPack(assetReference.getPart(GUID.UID_AS_VAULT).getBinaryData());
                        // logger.debug(" vault asset reference: "+vaultData.getXML());
                        // best to get via XML, but for now hack it via the display
                        // stirng
                        // String[] vaultBits
                        // =result.records[i].assetReferenceVault.split(", ");
                        // result.records[i].assetReferenceWindows =
