...
public class CumulusManager extends DamManager {
     ...
     * (non-Javadoc)
     *
     * @see com.acalian.dam.ws.DamManager#textSearch(java.lang.String, java.lang.String,
     * com.acalian.dam.common.SearchDescriptor)
     */
    @Override
    public QueryResult textSearch(Connection connection, String text, SearchDescriptor searchDescriptor, Locale locale) {
        ...
        try {
            c = collectionManager.getMasterServer().newMultiRecordItemCollection();
            // make sure we clone for now, as multi catalog collection close the added collections
            rc = (RecordItemCollection) collectionManager.cloneObjectToRead(RecordItemCollection.class);
            c.addItemCollection(rc);
    ...
    public byte[] previewFile(Connection connection, String id, Integer top, Integer left, Integer height, Integer width, Integer maxSize, Integer size, Integer rotate, String format, Integer quality, Path file, String actionName) {
        ...
        try {
            ...
            try {
                ...
                /*
                 * TODO see if we can sort this out
                 * I don't see how we can do this, as ImagingPixMap only works on RecordItems, and asset actions return physical files
                 * So the preview would have to be cataloged to do this, which is not on
                 if (actionName != null && !"".equals(actionName)) {
                 Path processedFile = CumulusUtilities.doAssetAction(connection.getName(), recordItem, actionName, tmpDir);
                 }
                 */
        ...
        try {
            ...
            if (Files.exists(file)) {
                ...
                if (tmpDir.compareTo(parentFolder.getParent()) == 0) {
                    // file created in sup folder in tmp, and should go - have been done as GUID
