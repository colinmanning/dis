	...
	public void init() {
		...
		for (Map.Entry<String, String> param : params.entrySet()) {
			if (param.getKey().startsWith(mapKey)) {
				...
				if (damBits.length > 3) {
					try {
						if (dateTimeFormatString != null && PARAM_BIT_STRING_TO_DATETIME.equals(damBits[3])) {
							fieldMap.setDateTimeFormat(new SimpleDateFormat(dateTimeFormatString));
							fieldMap.setStringToDateTime(true);
						}
					} catch (Exception e) {
						logger.error("Problem setting date format for db field: " + fieldMap.getDamField());
						e.printStackTrace();
					}
				}
	...
	public JSONObject execute(HashMap<String, String> urlParams) {
		...
		try {
			...
			for (Map.Entry<String, String> entry : urlParams.entrySet()) {
				logger.info("   --- url param: " + entry.getKey()
						+ " has value: " + entry.getValue());
				...
				} else if (BaseServlet.PARAMETER_CONNECTION_USERNAME
						.equals(entry.getKey())) {
					connUsername = entry.getValue();
				} else if (BaseServlet.PARAMETER_CONNECTION_PASSWORD
						.equals(entry.getKey())) {
					connPassword = entry.getValue();
				} else if (BaseServlet.PARAMETER_CONNECTION_POOLSIZE
						.equals(entry.getKey())) {
					connPoolSize = new Integer(entry.getValue());
				}
			}
			if (dbConnectionName == null || damConnectionName == null
					|| damConnection == null) {
			...
			if (connUsername != null && connPassword != null) {
				if (damManager.openConnection(damConnection, connUsername,
						connPassword, connPoolSize)) {
					tmpConnection = true;
				} else {
			...
			damQuery = buildNamedQuery(damNamedQuery, ignoreParams,
					damQueryParams);
			...
			QueryResult qr = damManager.querySearch(damConnection, null,
					damQuery, searchDescriptor, Locale.getDefault());
			...
			for (Record damRecord : qr.getRecords()) {
				...
				String keyFieldValue = damRecord.getFieldValue(damKeyField)
						.toString();
				if (keyFieldValue == null || "".equals(keyFieldValue)) {
					// ignore null or empty values
					continue;
				}
				logger.info("Processing DAM record with id: "
						+ damRecord.getId());
				dbQueryParams.put(dbKeyField, keyFieldValue);
				dbQuery = buildNamedQuery(dbNamedQuery, ignoreParams,
						dbQueryParams);
				logger.info("      --- db query: " + dbQuery);
				logger.info("      --- db view name: " + dbViewName);
				QueryResult dbQr = dbManager.fetch(dbConnectionName, dbQuery,
						dbSearchDescriptor, null);
				logger.info("      --- db query records returned: "
						+ dbQr.getRecords().length);
				if (dbQr.getRecords().length > 0) {
					// actually should be only 1, but we will take the first one
					// anyway, and log an issue
					if (dbQr.getRecords().length > 1) {
						logger.info("WARNING - Query: '"
								+ dbQuery
								+ "' returned more than one result, which is not good - maybe de-duping required");
					}
					Record dbRecord = dbQr.getRecords()[0];
					HashMap<String, String> damFields = new HashMap<>();
					for (FieldMap fieldMap : fieldMaps) {
						String v = dbRecord
								.getFieldValue(fieldMap.getDbField())
								.toString();
						if (fieldMap.isAppend()) {
							String exstingV = damRecord.getFieldValue(
									fieldMap.getDamField()).toString();
							v = exstingV + " " + v;
						}
						if (fieldMap.isStringToDateTime()) {
							// converting from a database String, to a DAM date
							// field, with formatter specified, or it will be
							// broken
							logger.info("Got date time value: " + v);
							v = (Long.valueOf(fieldMap.getDateTimeFormat()
									.parse(v).getTime())).toString();
							logger.info("Got long value of: " + v
									+ " to convert to date for db field: "
									+ fieldMap.getDbField() + " to DAM field: "
									+ fieldMap.getDamField());
						}
						damFields.put(fieldMap.getDamField(), v);
					}
					if (!damManager.updateAssetData(damConnection,
							damRecord.getId(), null, damFields)) {
						logger.error("Problem updating DAM record for key field: '"
								+ damKeyField
								+ "' with value '"
								+ keyFieldValue + "'");
					}
		...
		} catch (JSONException | DamManagerNotImplementedException
